...
import gym
import yaml
from gymai.agent import A2CAgent, create_converter
...
def main(args):
    ...
    converter = create_converter(conf["converter"], env.observation_space.shape)
    ...
    agent = A2CAgent(env, model, converter, hacks, render=args.render, **conf["agent"])
    while True:
        mems = agent.run()
